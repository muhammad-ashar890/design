...
7. **API key:** Site configuration → **Environment variables** → Add `AI_API_KEY` = your Gemini API key (aistudio.google.com/app/apikey). Optional: `AI_MODEL` (default: `gemini-1.5-flash`).
8. Click **Deploy**.
...
- Netlify functions have a time limit (10 s by default, up to 26 s). The default model `gemini-1.5-flash` is fast; a slower model may time out.
